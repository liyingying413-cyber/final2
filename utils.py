...
def analyze_with_openai(city: str, memory: str):
    ...
    如果环境变量中没有 OPENAI_API_KEY，则返回 None，由前端使用 fallback。
    """
    if client is None:
        return None

    instructions = (
        "You are an art director. Analyze the user's memory text about a city and "
        "output clean JSON design parameters for an abstract emotional poster. "
        "The poster is minimalist, atmospheric, with soft gradients and cinematic mood."
    )
